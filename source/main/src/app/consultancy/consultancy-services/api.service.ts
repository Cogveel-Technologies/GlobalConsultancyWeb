...
export class ConsultancyApi {
    ...
    getConsultancy(data:ConsultancyDetailsOptions):Observable<ConsultancyData[]>{
        return this.http.get<ConsultancyData[]>(`${this.baseUrl}/Consultancy?limit=${data.limit}&OrderBy=${data.OrderBy}&sortExpression=${data.sortExpression}&CurrentPage=${data.CurrentPage}`).pipe(map(response => response['data']))
    }
